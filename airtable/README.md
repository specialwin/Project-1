...
## 6) สรุปยอดขายรายเดือน + เทียบรายวัน

`08-monthly-sales.js` สรุปยอดทั้งเดือน พร้อมแตกยอดรายวันให้เห็นว่าวันไหนขายดี/น้อย
และจัดอันดับยาขายดี — รันเป็น Automation ตั้งเวลา (เช่นทุกสิ้นเดือน) หรือกดรันเองใน
Scripting extension ก็ได้

- **Input (ไม่บังคับ):** `monthISO` = `"YYYY-MM"` (ไม่ใส่ = เดือนปัจจุบัน)
- **Output:** `summary` · `month` · `bills` · `totalQty` · `revenue` · `activeDays`
- รายงาน: จำนวนบิล/หน่วย · ยอดรวม · เฉลี่ยต่อวัน · วันขายดีสุด · ตารางเทียบรายวัน ·
  ยาขายดี 10 อันดับ
- เกณฑ์เดียวกับรายวัน: นับใบ `Type=Sale` ที่ `Status=Done` · ยอดเงินจาก
  `Orders.Total` (หรือ `Order Items.Line Total`)

---

## 7) รายงานสต๊อกคงเหลือ + มูลค่าคงคลัง

`09-stock-report.js` รวมยอดคงเหลือจากตาราง **Lots** ตามรายยาและรายคลัง คำนวณ
มูลค่าคงคลัง และเตือนของที่ต่ำกว่าขั้นต่ำ/หมดอายุค้าง — กดรันเองใน Scripting
extension หรือตั้งเป็น Automation ส่งอีเมลก็ได้

- **Output:** `summary` · `totalQty` · `totalValue` · `lowStockCount` · `expiredQty`
- รายงาน: มูลค่าคงคลังรวม · จำนวนหน่วยคงเหลือ · แยกตามคลัง · รายการคงเหลือรายยา
  (พร้อมมูลค่า) · ⚠️ ต่ำกว่าขั้นต่ำ · 🛑 หมดอายุค้างสต๊อก
- ใช้ฟิลด์ทางเลือก: `Drugs.Price` (คิดมูลค่า — มูลค่าคิดเฉพาะของที่ยังไม่หมดอายุ),
  `Drugs.Min Qty` (เตือนของใกล้หมด), `Drugs.Unit` (หน่วยนับ); ไม่มีก็รายงานแค่จำนวน
